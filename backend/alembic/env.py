...
Reads the database URL from VECTIS settings (not from alembic.ini) so migrations
always target the same database as the application, and uses the ORM metadata as
the autogenerate target.
"""
...
from vectis.database import models  # noqa: F401

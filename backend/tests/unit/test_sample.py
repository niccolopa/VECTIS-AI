...
from vectis.data.regions import LIGURIA
from vectis.scripts.generate_sample import build_frame


def test_sample_is_reproducible() -> None:
    a = build_frame(LIGURIA, seed=42)
    b = build_frame(LIGURIA, seed=42)
    assert a.equals(b)


def test_sample_shape_and_columns() -> None:
    df = build_frame(LIGURIA, seed=42)
    assert len(df) == LIGURIA.n_cells
